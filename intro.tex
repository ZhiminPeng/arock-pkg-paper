...
\textbf{[Motivation for fixed-point problem]} Coordinate update (CU) methods reduce a large problem to smaller subproblems and are useful for solving large-sized problems.
...
\textbf{[Motivation for CU package]} Existing CU solvers \cite{,,,,} either focus on very particular problems or do not scale to large-sized problems due to their sequential implementation. Adapting them to solve slightly different large-sized problems requires significant human effort. 

\textbf{[Contribution of our work]} In this paper, we present ARock, a C++ library to simplify the implementation of both sequential and parallel coordinate update algorithms. The library reduces the barrier to entry on doing async-parallel computing. Users of ARock can use the prebuilt solvers or build their own async-parallel solvers by knowing a few concepts of classic optimization formulations and algorithms.
