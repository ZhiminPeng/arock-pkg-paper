...
\begin{equation}\label{eq:fbs_l1_log}
  x^{k+1} = \underbrace{\prox_{ \eta \lambda \|\cdot\|_1}}_{\text{backward operator}}
  (\underbrace{x^k - \eta \, \nabla_{x} \,\sum_{i = 1}^m \log (1 + \exp(-b_i \cdot a_i^T x^k)}_{\text{forward operator}}),
\end{equation}  
...
\begin{algorithm}[H]\label{alg:fbs_l1_log}
  ...
  \quad $\text{backward}(x) := \prox_{\lambda \eta \|\cdot\|_1} (x)$ \hspace{33.5mm} // backward operator\\
  \quad $\text{fbs}(x) := \text{backward}(\text{forward}(x))$  \hspace{30mm} // foward backward splitting \\
...
Similar substitutions will result in a solver for the following Lasso problem
